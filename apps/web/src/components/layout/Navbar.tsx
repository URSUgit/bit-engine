  ...
  return (
    <header className="h-14 shrink-0 flex items-center gap-3 px-4 bg-slate-950 border-b border-slate-800 relative z-30">
      <div className="flex-1 max-w-md">
        <button className="flex items-center gap-2 px-3 py-1.5 rounded-lg bg-slate-900 border border-slate-800 text-sm text-slate-500 w-full hover:bg-slate-800/80 hover:text-slate-300 transition-colors">
      ...
      <div className="ml-auto flex items-center gap-2">
        <button className="relative w-9 h-9 flex items-center justify-center rounded-lg hover:bg-slate-900 transition-colors">
          <Bell className="w-4 h-4 text-slate-400" />
          <span className="absolute top-1.5 right-2 w-1.5 h-1.5 rounded-full bg-cyan-500 animate-pulse-soft" />
        </button>
          ...
          {userOpen && (
            <>
              ...
              <div className="absolute right-0 top-11 z-40 w-56 bg-slate-900 border border-slate-800 rounded-xl shadow-2xl py-1 overflow-hidden">
                ...
                {["Profile", "Settings", "API Keys", "Billing"].map((label) => (
                  <button
                    key={label}
